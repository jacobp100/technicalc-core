...
let unitToStringPlural = (unit: unitType) =>
  ...
  | Foot => "Feet"
  | Celsius
  | Fahrenheit => unitToString(unit)
  | _ => unitToString(unit) ++ "s"
  };

let%private unitPartToString = (~plural, {prefix, unit, power}: unitPart) => {
  let out = plural ? unitToStringPlural(unit) : unitToString(unit);

  let out =
    switch (prefixToString(prefix)) {
    | Some(prefix) =>
      let secondLetter = Js.String.get(out, 1);
      let firstWordIsUpperCase =
        secondLetter == Js.String.toUpperCase(secondLetter);
      ...
      if (firstWordIsUpperCase) {
        ...
        prefix
        ++ Js.String.toLowerCase(firstLetter)
        ++ Js.String.sliceToEnd(~from=1, out);
      };
    | None => out
    };
...
let prefixValueToMml = (prefix: prefix) => {
  ...
  | (Some(base), Some(superscript)) =>
    Some("<msup><mn>" ++ base ++ "</mn><mn>" ++ superscript ++ "</mn></msup>")
  | _ => None
  };
};
