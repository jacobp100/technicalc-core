...
test("possible solver bug report", (. ()) => {
  ...
  let root = AST.solveRoot(~config=defaultConfig, ~context=AST_Context.empty, equation, ofFloat(0.))
...
test("solver bug report 2", (. ()) => {
  ...
  let root = AST.solveRoot(~config=defaultConfig, ~context=AST_Context.empty, equation, ofInt(1))
