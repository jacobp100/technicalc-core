...
%%private(
  let unitPartValue = (~powerMultiplier=1, {prefix, unit, power}) => {
    ...
    open Decimal
    (ofFloat(prefixValue(prefix)) * ofFloat(unitLinearValueExn(unit))) ** ofInt(nextPower)
  }
)
...
%%private(
  let transformUnits = (
    ...
    switch units {
    | [{prefix, unit: Celsius, power: 1}] => transformCelsius(prefixValue(prefix)->ofFloat * value)
    | [{prefix, unit: Fahrenheit, power: 1}] =>
      transformFahrenheit(prefixValue(prefix)->ofFloat * value)
