...
let rec eval = (~config, ~context, ~x, node: t): Value.t =>
  ...
  | Percent(percent) =>
    evalScalar(~config, ~context, ~x, percent)->Value.ofPercent
  | Vector(elements) =>
    Value.ofVector(
      Belt.Array.map(elements, evalScalar(~config, ~context, ~x)),
    )
  ...
  | Add(a, b) =>
    Value.add(
      eval(~config, ~context, ~x, a),
      eval(~config, ~context, ~x, b),
    )
  | Sub(a, b) =>
    Value.sub(
      eval(~config, ~context, ~x, a),
  ...
  | Cosec(a) =>
    eval(~config, ~context, ~x, a)->toRad(~config, _)->Value.cosec
  ...
  | Asinh(a) =>
    eval(~config, ~context, ~x, a)->Value.asinh->ofRad(~config, _)
  | Acos(a) => eval(~config, ~context, ~x, a)->Value.acos->ofRad(~config, _)
  | Acosh(a) =>
    eval(~config, ~context, ~x, a)->Value.acosh->ofRad(~config, _)
  | Atan(a) => eval(~config, ~context, ~x, a)->Value.atan->ofRad(~config, _)
  | Atanh(a) =>
    eval(~config, ~context, ~x, a)->Value.atanh->ofRad(~config, _)
  | ToRad(a) => eval(~config, ~context, ~x, a)->toRad(~config, _)
  | ToDeg(a) =>
    eval(~config, ~context, ~x, a)->toRad(~config, _)->Value.toDeg
  | ToGrad(a) =>
    eval(~config, ~context, ~x, a)->toRad(~config, _)->Value.toGrad
  | OfRad(a) => eval(~config, ~context, ~x, a)->ofRad(~config, _)
  | OfDeg(a) =>
    eval(~config, ~context, ~x, a)->Value.ofDeg->ofRad(~config, _)
  | OfArcMin(a) =>
    eval(~config, ~context, ~x, a)->Value.ofArcMin->ofRad(~config, _)
  | OfArcSec(a) =>
    eval(~config, ~context, ~x, a)->Value.ofArcSec->ofRad(~config, _)
  | OfGrad(a) =>
    eval(~config, ~context, ~x, a)->Value.ofGrad->ofRad(~config, _)
...
let eval = (~context, ~config, v) =>
  eval(~config, ~context, ~x=Value.nan, v);
...
let solveVar3 =
    (~config, ~context, x0, y0, z0, c0, x1, y1, z1, c1, x2, y2, z2, c2) => {
  let x0 = eval(~config, ~context, x0);
  let y0 = x0 != `N ? eval(~config, ~context, y0) : `N;
  let c0 = y0 != `N ? eval(~config, ~context, c0) : `N;
