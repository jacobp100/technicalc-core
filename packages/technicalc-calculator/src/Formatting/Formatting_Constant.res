...
let toString = (~format, a) => {
  ...
  | (Some(Unicode), Exp(v)) => "e" ++ f(format, v)->Formatting_Unicode.formatSuperscriptNumbers
  | (Some(Unicode), Sqrt(v)) => `${Formatting_Unicode.sqrt}(${f(format, v)})`
  | (Some(Tex), Pi(1)) => "\\pi"
