...
%%private(
  let ofUnit = ({prefix, name, power}: TechniCalcCalculator.Units.t) => {
...
let ofValue = (a: TechniCalcCalculator.Value.t) =>
  ...
  | #Vect(elements) =>
    Belt.Array.reduce(elements, [TableNS({numRows: Belt.Array.length(elements), numColumns: 1})], (
      accum,
      element,
    ) => Belt.Array.concatMany([accum, ofFinite(element), [Arg]]))
  | #Matx({numRows, numColumns, elements}) =>
    Belt.Array.reduce(elements, [TableNS({numRows, numColumns})], (accum, element) => {
      Belt.Array.concatMany([accum, ofFinite(element), [Arg]])
    })
  | #Mesr({value, units}) => Belt.Array.concat(ofReal(value), Belt.Array.flatMap(units, ofUnit))
  }
