...
%%private(
  let table = (~numRows, ~numColumns, elements, superscript) => {
    let body = Belt.Array.makeBy(numRows, row => {
...
module Tex_Accum = Stringifier.Make({
  ...
  let groupingSeparator = groupingSeparator => groupingSeparator == " " ? `\\,` : groupingSeparator
  let decimalSeparator = (decimalSeparator, _, _) => decimalSeparator

  let unpairedOpenBracket = (_, _) => " ( "
  let unpairedCloseBracket = (superscript, _, _) => withSuperscript(" ) ", superscript)

  let bracketRange = (superscript, body, _, _, _) =>
    withSpaces(`\\left( ${body} \\right)`)->withSuperscript(superscript)
})

%%private(
  let supsrscriptSuffix = (accum, body) =>
    Tex_Accum.modifyLast(accum, last => {
...
let create = (~format, elements) =>
  if Belt.Array.length(elements) != 0 {
    AST.reduceMap(elements, ~reduce, ~map, ~initial=Tex_Accum.make(format))->removeBraces
  } else {
    ""
  }
