...
test("parses numbers", (. ()) => {
  ...
  parseEval([N0_S, DecimalSeparator, N5_S])->expect->toEqual(Some(ofString("0.5")))

  parseEval([DecimalSeparator, N5_S])->expect->toEqual(Some(ofString("0.5")))

  parseEval([N5_S, DecimalSeparator])->expect->toEqual(Some(ofString("5")))
...
test("parses imaginary units", (. ()) => {
  ...
  parseEval([N2_S, ImaginaryUnitS])->expect->toEqual(Some(ofString("2")->mulI))

  parseEval([N1_S, N0_S, ImaginaryUnitS])->expect->toEqual(Some(ofString("10")->mulI))
  ...
  parseEval([N1_S, N0_S, Superscript1, N2_S, Arg, ImaginaryUnitS, Superscript1, N2_S, Arg])
  ->expect
...
test("angles", (. ()) => {
  ...
  parseEval([N1_S, DegreeUnit])->expect->toEqual(Some(div(pi, ofInt(180))))

  parseEval([N1_S, ArcMinuteUnit])->expect->toEqual(Some(div(pi, ofInt(180 * 60))))

  parseEval([N1_S, ArcSecondUnit])->expect->toEqual(Some(div(pi, ofInt(180 * 60 * 60))))
  ...
  parseEval([N1_S, ArcMinuteUnit, N1_S, DegreeUnit])->expect->toEqual(None)
